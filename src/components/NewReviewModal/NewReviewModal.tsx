...
export const NewReviewModal = () => {
    ...
    return (
        <>
            {isTagsModalOpen ? null : <div className={styles.modal}>
                ...
                <div className={styles.modal__mid}>
                    <textarea onChange={(e) => setReview(e.target.value)} placeholder='Start writing here' value={review} className={styles.textarea}  cols={30} rows={10}></textarea>
                    <div className={styles.ratingcontainer}>
                        <Rate allowHalf={true} value={rating} onChange={(e) => {
                            setRating(e)
                        }}/>
                    </div>
                </div>
                <div className={styles.modal__bottom}>
                    <div className={styles.bottom__top}>
                        <div className={styles.checkcontainer}>
                            <input checked={tags?.length > 0 ? true : false} onChange={() => setIsTagsModalOpen(!isTagsModalOpen)} type="checkbox" />
                            <span>Select Review Tags</span>
                        </div>
                        <div className={styles.checkcontainer}>
                            <input onChange={() => setIsAnonymous(!isAnonymous)} type="checkbox" />
